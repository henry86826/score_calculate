...
function setGPA() {
  //抓到每個表格長度(總共有幾個表格)
  let formLength = document.querySelectorAll("form").length; //NodeList可直接接長度
  //學分
  let credits = document.querySelectorAll(".class-credit");
  //成績下拉選單
  let select = document.querySelectorAll("select");

  let sum = 0; //GPA計算用分子，分子 = 學分 * 加權
  let creditSum = 0; //GPA計算用分母，分母 = 學分總數
...
function handleSorting(direction) {
  ...
  //取得object array後，我們可以把成績String(A+...)換成數字
  //用convertor()
  for (let i = 0; i < objectArray.length; i++) {
    objectArray[i].class_grade_number = convertor(objectArray[i].class_grade);
  }

  //objectArray已經是push完後的狀態，用mergeSort()來排序他
  objectArray = mergeSort(objectArray);
  if (direction == "descending") {
    objectArray = objectArray.reverse();
  }
  //根據object array內容更新網頁
  ...
  for (let i = 0; i < objectArray.length; i++) {
    ...
    //因為 select 無法用上面的string做更改，因此用JS更改
  }
  // 更改select部分
  //重新選一次div.grader
  ...
  allTrash.forEach((trash) => {
    trash.addEventListener("click", (e) => {
      ...
      e.target.parentElement.parentElement.addEventListener(
        "animationend",
        (e) => {
          //e.target = parent的parent element
          e.target.remove();
          setGPA();
        }
      );
  ...
  console.log(mergeSort(objectArray));
}
